...
//  encoder.reset();
  ...
  while (fabs(controller.getError()) > 3) {
    ...
    // ACTION: Detect left wall using LiDar & IMU
    ...
    float heading = getYawMPU();

    while (heading - startingYaw > 0.9 && leftWall > 85) {
      motor1.setPWM(-controlSignal/3);
      motor2.setPWM(-controlSignal/3);
      
      leftWall = sensor1.readRangeSingleMillimeters();
      heading = getYawMPU();
    }

    while (heading - startingYaw < -0.9 && rightWall > 85) {
      motor1.setPWM(controlSignal/3);
      motor2.setPWM(controlSignal/3);
...
//    turnRight(-(getYawMPU() - startingYaw), 2);  
//  } 
//  
//  if (getYawMPU() - startingYaw < -2) {
//    turnLeft(getYawMPU() - startingYaw, 2);  
//  } 
