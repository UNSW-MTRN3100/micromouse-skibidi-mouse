...
// Maze dimensions
const int rows = 9;  // 9 rows from 0 to 8
const int cols = 5;  // 5 columns from 0 to 4
...
int startX = 0;
int startY = 0;
int endX = 8;  // Corrected to 8 to represent the bottom-right corner
int endY = 4;  // Corrected to 4 to represent the bottom-right corner
...
// Direction arrays (initially facing downward)
int dirX[] = {0, 1, 0, -1}; // Down, Up, Right, Left
int dirY[] = {1, 0, -1, 0};
...
void Task3_4() {
    ...
    // Step 2: Ensure all cells are visited
    ensureFullCoverage();
...
void exploreMaze() {
    ...
    while (visitedCells < totalCells) {
        ...
        // Display the mapping progress on OLED
        ...
        // Measure distances using the LIDAR sensors
        ...
        if (frontDistance < 150) { 
            int newX = posX + dirX[1]; // Potential next X position
            int newY = posY + dirY[1]; // Potential next Y position
            if (newX >= 0 && newX < rows && newY >= 0 && newY < cols) {
                maze[newX][newY] = 1; // Mark front as obstacle
            }
        }
        if (rightDistance < 150) { 
            int newX = posX + dirX[3]; // Potential right X position
            int newY = posY + dirY[3]; // Potential right Y position
            if (newX >= 0 && newX < rows && newY >= 0 && newY < cols) {
                maze[newX][newY] = 1; // Mark right as obstacle
            }
        }
        if (leftDistance < 150) { 
            int newX = posX + dirX[2]; // Potential left X position
            int newY = posY + dirY[2]; // Potential left Y position
            if (newX >= 0 && newX < rows && newY >= 0 && newY < cols) {
                maze[newX][newY] = 1; // Mark left as obstacle
            }
        }
        ...
        if (frontDistance >= 150) {
            ...
            delay(500);
            stopMotors();
            updatePosition(); // Update posX and posY based on the direction
        } else if (rightDistance >= 150) {
            // If there is a wall in front but not on the right, turn right
            turnRight();
            moveForward();
            delay(500);
            stopMotors();
            ...
            // If there is a wall in front but not on the left, turn left
            turnLeft();
            moveForward();
            delay(500);
            stopMotors();
...
void updatePosition() {
    if (dirX[1] == 1 && posX < rows - 1) { // Moving downwards, within bounds
        posX++;
    } else if (dirX[0] == -1 && posX > 0) { // Moving upwards, within bounds
        posX--;
    } else if (dirY[1] == 1 && posY < cols - 1) { // Moving right, within bounds
        posY++;
    } else if (dirY[2] == -1 && posY > 0) { // Moving left, within bounds
        posY--;
    }
}
...
void ensureFullCoverage() {
    for (int i = 0; i < rows; i++) {
        for (int j = 0; j < cols; j++) {
            if (!visited[i][j]) {
                bfs(posX, posY, i, j); // Run BFS to the missed cell
                followPath(i, j);
            }
        }
    }
...
void turnRight() {
    ...
    updateDirection(1); // Update direction after turning right
}
...
void turnLeft() {
    ...
    driveStop();  
    encoder_odometry.reset();
    updateDirection(-1); // Update direction after turning left
}

void updateDirection(int turn) {
    // Adjust the robot's direction based on the turn
