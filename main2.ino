 ...
 // processCommands(commands);
  ...
  if (flag == 0 ){
   ...
   // performActionBasedOnString("Task3_1"); 
    performActionBasedOnString("Task3_4"); 
  }
...
void exploreMaze() {
    for (int i = 0; i < rows; i++) {
        for (int j = 0; j < cols; j++) {
            // Measure distance from front, right, and left sensors
            ...
            if (frontDistance < 150) { // If there is a wall in front
                maze[i][j] = 1;
            }
            if (rightDistance < 150) { // If there is a wall on the right
                if (j < cols - 1) {
                    maze[i][j + 1] = 1;
                }
            }
            if (leftDistance < 150) { // If there is a wall on the left
                if (j > 0) {
                    maze[i][j - 1] = 1;
                }
            }
            ...
            moveForward();
            delay(500); // Move forward for a certain time
            stopMotors();
        }
        String mapProcess = "Map process: %" + String(i);
        //write process 
        display.clearDisplay();
        writeTextToOLED(mapProcess, 0, 0);
        // End of a row, change direction
        if (i % 2 == 0) {
            turnRight();
        } else {
            turnLeft();
...
void bfs(int startX, int startY) {
    ...
    // Initialize all cells' distances and parents to -1
    for (int i = 0; i < rows; i++) {
        for (int j = 0; j < cols; j++) {
            dist[i][j] = -1;
            parentX[i][j] = -1;
    ...
    while (!queue.isEmpty()) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            // If the cell is within the maze and not visited yet
    ...
    while (pathX != startX || pathY != startY) {
        ...
        // Determine the direction of movement and control the motors
        if (prevX == pathX - 1) {
            moveForward();
            delay(500);
